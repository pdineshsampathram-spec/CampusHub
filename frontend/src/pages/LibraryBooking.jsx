...
export default function LibraryBooking() {
  ...
  const fetchBookedSeats = useCallback(async () => {
    try {
      const { data } = await libraryService.getSeatsStatus(selectedDate, selectedZone.floor);
      setBookedSeats(data);
    } catch {
      setBookedSeats([]);
    }
        ...
        <div className="xl:col-span-2 glass-card p-6 space-y-5">
          ...
          <div>
            <h3 className="font-semibold text-slate-700 dark:text-slate-200 mb-3 flex items-center gap-2">
              <BookOpen size={16} className="text-indigo-500" /> Floor {selectedZone.floor} Seat Map
            </h3>
